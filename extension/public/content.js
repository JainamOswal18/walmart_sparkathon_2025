    ...
    ...(typeof productIdentifier === "string" && !isNaN(productIdentifier)
      ? Array.from(
          document.querySelectorAll(
            `[data-product-id="${productIdentifier}"] button`
          )
        )
      : []),
    // By product name
    ...Array.from(
      document.querySelectorAll(".product-card, .product-item, [data-product]")
    )
      .filter((card) => {
...
function navigateTo(destination) {
  ...
  const navLinks = Array.from(
    document.querySelectorAll("nav a, header a, .navigation a")
  ).filter((link) =>
    link.textContent.toLowerCase().includes(destination.toLowerCase())
  );
...
function filterProducts(filters) {
  ...
  Object.entries(filters).forEach(([filterName, filterValue]) => {
    ...
    const filterElements = Array.from(
      document.querySelectorAll(".filter, .refinement, [data-filter]")
    ).filter((el) =>
      el.textContent.toLowerCase().includes(filterName.toLowerCase())
    );
    ...
    if (filterElements.length > 0) {
      ...
      setTimeout(() => {
        const filterOptions = Array.from(
          ...
          return (
            label &&
            label.textContent.toLowerCase().includes(filterValue.toLowerCase())
          );
        });
...
function selectProduct(identifier) {
  ...
  if (
    ...
    /^(first|second|third|fourth|fifth|1st|2nd|3rd|4th|5th)$/i.test(identifier)
  ) {
    const index = {
      first: 0,
      "1st": 0,
    ...
    const productElements = document.querySelectorAll(
      ".product-card, .product-item, [data-product]"
    );
